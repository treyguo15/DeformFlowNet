...
class Backbone(nn.Module):
    ...
    arch_settings = {
        9: (ResBlock, (3, 3, 2)),8: (ResBlock, (2, 2, 2))
    }
    ...
    def __init__(self,
        ...
        self.inplanes = 48
        self.conv1 = conv3x3x3(in_channels, 48, kernel_size=7, stride=(1, 2, 2), padding=3, bias=False, weight_std=weight_std)
        self.norm1 = Norm_layer(norm_cfg, 48)
        self.nonlin = Activation_layer(activation_cfg, inplace=True)
        self.layer1 = self._make_layer(block, 96, layers[0], stride=(2, 2, 2), norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std)
        self.layer2 = self._make_layer(block, 192, layers[1], stride=(2, 2, 2), norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std)
        self.layer3 = self._make_layer(block, 768, layers[2], stride=(2, 2, 2), norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std)
        self.layer4 = self._make_layer(block, 768, layers[2], stride=(2, 2, 2), norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std)
        self.layers = []
#192,384,384
...
if __name__=='__main__':
    ...
    import torch
    x = torch.ones(4, 1,4, 224, 224).cuda() #batchsize, channel,depth, h,w

    print (net.forward(x)[0].size())
    print (net.forward(x)[1].size())
    print (net.forward(x)[2].size())
    #print (net.forward(x)[3].size())
    # print (net.forward(x)[4].size())

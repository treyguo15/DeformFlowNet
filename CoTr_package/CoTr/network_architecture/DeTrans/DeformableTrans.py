...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds):
        ...
        #print(src_flatten.size(),spatial_shapes,lvl_pos_embed_flatten.size(),level_start_index,valid_ratios,mask_flatten.size())
        # encoder
...
if __name__=='__main__':
    ...
    net = DeformableTransformer(d_model=384, dim_feedforward=1536, dropout=0.1, activation='gelu', num_feature_levels=1, nhead=6, num_encoder_layers=6, enc_n_points=4)#.cuda()

    import torch
    
    #x1 = torch.randint(0,2,(4, 384,2, 14, 14))#.cuda()
    x1 = torch.ones(4, 384,2, 14, 14)
    x2 = torch.ones(4, 384,1, 7, 7)#.cuda()#batchsize, channel,depth, h,w
    x=[x1,x2]
    y1= torch.zeros(4, 2, 14, 14,dtype=torch.bool)#.cuda()
    y2= torch.zeros(4, 1, 7, 7,dtype=torch.bool)#.cuda()
    y=[y1,y2]
    z1 = torch.ones(4, 384,2, 14, 14)#.cuda()
    z2= torch.ones(4, 384,1, 7, 7)#.cuda()
    z=[z1,z2]
    
    x=[x1]
    y=[y1]
    z=[z1]
    print (net.forward(x,y,z).size())
